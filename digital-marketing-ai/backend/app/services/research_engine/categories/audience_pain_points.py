...
SYSTEM_PAIN_POINTS = """
You analyze live sources to extract audience pain points and customer language.
Rules:
- Use ONLY the provided sources.
- Do NOT invent pain points.
- Every extracted pain point/language item must cite source_indexes.
...
async def run(
    *,
    ...
    category_key = "audience_pain_points"
    category_name = "Audience & Pain-Point Research"

    idea = (state.extracted.product_or_service if state.extracted else None) or (state.raw_input or "")
    target = (state.extracted.target_audience if state.extracted else None) or ""
    location = (state.answers.get("target_market_geo") if state.answers else None) or ""

    if not idea.strip() or not target.strip():
        return ResearchCategoryResult(
            category_key=category_key,
            category_name=category_name,
            status="insufficient_data",
            confidence=0.0,
            summary="Need both offer and target audience to research pain points.",
        )

    queries = [
        f"{target} problems with {idea}".strip(),
        f"{idea} complaints".strip(),
        f"reddit {idea} problems".strip(),
    ]
    ...
    if not all_sources:
        return ResearchCategoryResult(
            ...
            status="insufficient_data",
            confidence=0.2,
            summary="No reliable live sources found for audience pain points.",
    ...
    schema = {
        ...
        "findings": {
            "pain_points": [{"finding": "string", "source_indexes": [1], "confidence": 0.0}],
            "customer_language": [{"finding": "string", "source_indexes": [1], "confidence": 0.0}],
            "emotional_triggers": [{"finding": "string", "source_indexes": [1], "confidence": 0.0}],
            "unmet_needs": [{"finding": "string", "source_indexes": [1], "confidence": 0.0}],
        },
    }
...
""".strip()
    ...
    try:
        analysis = await llm.analyze_json(prompt=prompt, system=SYSTEM_PAIN_POINTS)
    except ResearchProviderError as exc:
        return ResearchCategoryResult(
            category_key=category_key,
            category_name=category_name,
